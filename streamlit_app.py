...
def _secret(key: str, default: str = "") -> str:
    """Try Streamlit secrets first, then env, then default."""
    try:
...
- regulation_summary: 2-3 sentence summary
- impact_score_overall: 0-100
- impacted_areas: array of objects with keys:
    name, type (Policy|Workflow|System), priority (High|Medium|Low),
    impact_reason, recommended_action, risk_if_not_implemented,
    confidence_score (0-1), supporting_citations (array of strings)
- citations: array with citation_id, source_title, section, snippet, relevance
"""

    result = call_llm(
        system="You are a healthcare regulatory analyst. Identify impacts of regulations on internal payer artifacts. Cite sources by chunk_id.",
        user=user_prompt,
        max_tokens=2500,
    )

    impact_score = int(result.get("impact_score_overall", 0))
...
- changes: array of objects with type (Added|Removed|Modified), section,
    description, old_text, new_text, compliance_risk_delta, operational_impact_delta,
    impact, recommended_action
"""

    result = call_llm(
        system="You compare regulatory documents and surface meaningful changes.",
...
""", unsafe_allow_html=True)
        ...
        with sc1:
            sim_btn = st.button(f"⚙ Simulate gap", key=f"sim_{i}",
                               use_container_width=True)
        if sim_btn:
            ...
            with st.expander(f"What-if: {ia['name']}", expanded=True):
                try:
                    sim = call_llm(
                        ...
                        user=f"Project consequences if gap unaddressed.\nRegulation: {res.get('regulation_summary')}\nGap: {ia.get('impact_reason')}",
                        max_tokens=600)
                    fe = sim.get("financial_exposure", {})
                    sm1, sm2, sm3 = st.columns(3)
                    sm1.metric("Financial low", f"${int(fe.get('low_estimate_usd', 0)):,}")
                    sm2.metric("Financial high", f"${int(fe.get('high_estimate_usd', 0)):,}")
                    sm3.metric("Likelihood", sim.get("likelihood_of_enforcement", "—"))
                    st.markdown(f"**Basis:** {fe.get('basis', '—')}")
                    st.markdown(f"**Regulatory:** {sim.get('regulatory_exposure', '—')}")
                    st.markdown(f"**Member impact:** {sim.get('member_impact', '—')}")
                    st.markdown(f"**Operational:** {sim.get('operational_friction', '—')}")
                    st.caption(f"Mitigation window: {sim.get('mitigation_window_days', 90)} days")
                except Exception as e:
...
""", unsafe_allow_html=True)
    ...
    for c in res["changes"]:
        ...
        old_html = (f'<div class="diff-removed">{c["old_text"][:600]}</div>'
                   if c.get("old_text") else
                   '<div style="color:var(--ink-mute); font-size:13px;">—</div>')
        new_html = (f'<div class="diff-added">{c["new_text"][:600]}</div>'
                   if c.get("new_text") else
                   '<div style="color:var(--ink-mute); font-size:13px;">—</div>')
        st.markdown(f"""
<div class="pane">
  <div class="pane-head">
